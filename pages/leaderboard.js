...
export default function Leaderboard() {
  ...
  const load = async () => {
    ...
    // Fetch all matches (needed for bracket computation + score checking)
    ...
    // Build per-user predMap for bracket computation
    const predsByUser = {};
    (preds || []).forEach((p) => {
    ...
    // Actual knockout teams per stage (from DB)
    ...
    // Determine the actual winner (from finished Final)
    ...
    // Score initial predictions against finished group matches
    const finishedGroupIds = finished.filter((m) => m.stage?.startsWith("Group ")).map((m) => m.id);
    (preds || []).forEach((p) => {
    ...
    // Score live predictions against finished knockout matches
    const livePtsMap = {};
    const liveExactMap = {};
    const liveResultMap = {};
    (livePreds || []).forEach((p) => {
    ...
    // Compute bracket points per user
    const bracketPtsMap = {};
    Object.keys(predsByUser).forEach((uid) => {
      ...
      knockoutKeys.forEach((key) => {
        ...
        if (key === "Final" && actualWinner) {
          ...
          if (finalBracket) {
            const predWinner = finalBracket.home_team; // not actually computable here without pred scores
            // Check if user's Final prediction winner matches actual winner
