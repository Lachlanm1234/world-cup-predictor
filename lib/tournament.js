...
function buildR32Teams(winners, runners, bestThirds) {
  ...
  // 4 runner vs runner: remaining annex runners paired with free group runners
  const remainingAnnexRunners = annexGroupsSorted.slice(freeGroups.length); // e.g. [E,F,G,H]
  const runnerRunnerPairs = remainingAnnexRunners.map((rg, i) => ({
    home: runners[rg] || "TBD",
    away: runners[freeGroups[i]] || "TBD",
  }));
